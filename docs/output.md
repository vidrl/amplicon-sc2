...
If Nextclade is optionally run with `--nextclade`, a folder `nextclade` is created in the output directory containing the nextclade dataset used and a tsv file for each sample. In addition, a table summarising the results are included in run report (`<SCHEME_NAME>_amplicon-nf_run-report.html`).